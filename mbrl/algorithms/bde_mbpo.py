...
def deepcopy_agent(agent: SACAgent, cfg):
    '''
    deep copy the agent
    '''
    agent_copy = SACAgent(
    ...
    agent_copy.sac_agent.policy = deepcopy(agent.sac_agent.policy)# .clone()
    agent_copy.sac_agent.critic = deepcopy(agent.sac_agent.critic)# .clone()
    # agent_copy.sac_agent.critic_optim = agent.sac_agent.policy.detach().clone()
    return agent_copy
    ...
    while env_steps < cfg.overrides.num_steps:
        ...
        for steps_epoch in range(cfg.overrides.epoch_length):
            ...
            if (env_steps + 1) % cfg.overrides.freq_train_model == 0:
                ...
                # log_model_weights_freq = 0 means not log model weights
                itr_flag = (env_steps + 1) // cfg.overrides.freq_train_model
                if log_model_weights_freq != 0:
                    log_model_weights = (itr_flag % log_model_weights_freq == 0)
                else: 
