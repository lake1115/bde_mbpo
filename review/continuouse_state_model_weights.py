...
if __name__ == "__main__":
    ...
    root_dir = 'review/'
    log_dir = osp(root_dir, f'continuous{seed}_rl{rollout_horizon}_idx{idx}_agent_{args.agent_type}')
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)

    # Get agent and model directory
    agent_dir, model_dir = get_model_dir(args.agent_type)

    df_denoise_name = osp(log_dir , f'idx{idx}_seed{seed}_rl{rollout_horizon}_df_denoise.csv')
    df_noise_name = osp(log_dir , f'idx{idx}_seed{seed}_rl{rollout_horizon}_df_noise.csv')
    ...
    for j in range(10):
        ...
        df_all.to_csv(osp(log_dir , f'model_weights, idx{idx+j}_seed{seed}_rl{rollout_horizon}.csv'), index=False)
        for i in range(5):
            plt.plot(df_all['epoch'], df_all[f'{i}'], label=f'{i}')
        plt.legend()
        plt.savefig(osp(log_dir , f'idx{idx+j}_seed{seed}_rl{rollout_horizon}.png'))
        plt.cla()
